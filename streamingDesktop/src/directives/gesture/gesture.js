function getLen (v) {
  ...
  export function getUserAgent () {
	try {
		  if (/Android|webOS|iPhone|iPad|iPhone OS|Phone|iPod|BlackBerry|iOS/i.test(navigator.userAgent)) {
			  return 'Mobile'
		  } else {
			  return 'PC'
		  }
  ...
  export default class Gesture {
	...
	start (evt) {
		  ...
		  this.inOrder = true
		  this.moveFlag = false
		  if (evt.touches && evt.touches[0].target.id === 'remoteVideo') {
			  if (this.userAgent === 'Mobile'|| this.userAgent === 'Mini') {
		  ...
		  if (evt.touches && evt.touches.length > 1) {
			  ...
			  this.count = evt.touches.length
			  if (this.count === 2 && evt.touches[0].target.id !== 'remoteVideo' && evt.touches[1].target.id === 'remoteVideo') {
				  this.holdClick = true
			  }
			  let FingerClickArray = Array.prototype.slice.call(evt.touches).map(item => item.target.id)
			  let FingerClickFlag = Array.prototype.slice.call(evt.touches).every(item => item.target.id === 'remoteVideo')
			  if (FingerClickFlag) {
				  if (FingerClickArray.length === 2) {
					  this.rightClick = true
				  } else if (FingerClickArray.length === 3) {
	...
	move (evt) {
		  ...
		  this.isDoubleTap = false;
		  if (evt.touches && evt.touches.length > 1) {
			  if (evt.touches[0].target.id === 'remoteVideo' && evt.touches[1].target.id === 'remoteVideo') {
				  ...
				  this.sx2 = sCurrentX;
				  this.sy2 = sCurrentY;
			  } else if (evt.touches[0].target.id === 'remoteVideo' && evt.touches[1].target.id !== 'remoteVideo') {
				  if (this.x2 !== null) {
				  ...
				  if (this.x2 !== null && this.sx2 !== null) {
					  evt.moveMouse = true
					  evt.deltaX = (sCurrentX - this.sx2);
					  evt.deltaY = (sCurrentY - this.sy2);
				  } else {
					  evt.tapTheScreen = true
					  evt.deltaX = 0;
					  evt.deltaY = 0;
				  }
				  this.Observer.dispatch('twoFingerPressMove', evt)
		  ...
		  if (!this.moveFlag) {
			  ...
			  if (this.holdClick) {
				  evt.holdClick = true
			  } else if (this.rightClick && !this.threeFingerClick && this.count === 2) {
				  evt.rightClick = true
			  } else if (this.threeFingerClick) {
				  evt.threeFingerClick = true
			  }
		  }
		  this.holdClick = false
		  this.rightClick = false
