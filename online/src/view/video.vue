      ...
      <van-overlay :show="show">
        <div class="wrapper">
          <div class="block" v-show="active == 1">
            <p class="choose" style="text-align: center; font-size: 18px">
              请选择操作模式
            </p>
            <van-row
              type="flex"
            ...
            <van-row type="flex" justify="space-between">
              <van-col span="11">
                <p style="font-size: 12px">
                  触控操作，易上手。但部分游戏模式操作困难。游戏时建议切换为鼠标模式。
                </p>
              </van-col>
              <van-col span="11">
                <p style="font-size: 12px">
                  以鼠标为主体的操作模式。适配性高，利于配合虚拟键盘对任意游戏进行操控。
                </p>
              </van-col>
            </van-row>
            <p
          ...
          <div class="mouse" v-show="active == 3">
            ...
            <div class="imgs" style>
              ...
              <img
                style="width: 18%; margin-left: 10vw; margin-top: 3vh"
                :src="guideList[9]"
              />
              <img
                style="width: 18%; margin-left: 10vw; margin-top: 3vh"
...
export default {
  ...
  watch: {
    // archive() {
    //   if (this.archive) {
    //     console.log('存档消息通知',new Date().getTime() - this.startTime)
    //     console.log('存档消息通知时间',new Date())
    //     let appData = sessionStorage.getItem("app");
    //     let dataMsg = util.decrypt(appData);
    //     console.log('watch',dataMsg)
    //     if (dataMsg && dataMsg.is_archive == 1) {
    //       this.isHref();
    //     }
    //   }
    // },
  ...
  mounted() {
    ...
    $(".btnIcon")[0].style.right = 66.7 + this.screen.left + "px";
  },
  beforeDestroy() {
    // 强制横屏test
    ...
    panmove(event) {
      ...
      if (this.isHorizontalScreen) {
        $(".btnIcon")[0].style.left =
          event.targetTouches[0].clientX  - 25 + "px";
        $(".btnIcon")[0].style.top =
          event.targetTouches[0].clientY  - 25 + "px";
      } else {
        $(".btnIcon")[0].style.left =
      ...
      if (iconLeft <= this.screen.left) {
        this.traceLog("iconLeft <= 0");
        $(".btnIcon")[0].style.left = this.screen.left + "px";
      }
      if (iconTop <= this.screen.top) {
        this.traceLog("iconTop  <= 0");
        $(".btnIcon")[0].style.top = this.screen.top + "px";
      }
      if (
        iconLeft >=
