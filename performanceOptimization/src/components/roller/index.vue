<template>
  <div class="roller" :style="{left: wheelLeft + 'px', top: wheelTop + 'px'}">
    <div
      class="wheel"
      ref="wheel"
...
export default {
  name: "Roller",
  props: ["screen", "isHorizontalScreen", "wheelLeft", "wheelTop"],
  data() {
    return {
      btn: 2,
      isVertical: true,
  ...
  methods: {
    ...
    initRoller() {
      ...
      const left = this.stripPX($(".roller").css("left"));
      const top = this.stripPX($(".roller").css("top"));
      const width = $(".wheel").width();
      const heigth = $(".wheel").height();
      ...
      const d_width = $("#directionBtn").width();
      const d_heigth = $("#directionBtn").height();
    ...
    moveBtn(e) {
      ...
      if (distance > this.radius) {
        ...
        directionInfo.style.left =
          this.dotX - this.stripPX($(".roller").css("left")) + "px";
        directionInfo.style.top =
          this.dotY - this.stripPX($(".roller").css("top")) + "px";
        return;
      }
      ...
      directionInfo.style.left =
        x -
        $("#directionBtn").width() / 2 -
        this.stripPX($(".roller").css("left")) +
        "px";
      directionInfo.style.top =
        y -
        $("#directionBtn").height() / 2 -
        this.stripPX($(".roller").css("top")) +
        "px";
      // console.log(
      //   "中心位置",
      //   directionInfo.style.left,
    ...
    circularMotion(non_dx, non_dy) {
      ...
      if (non_dx > 0) {
        ...
        // 第二和第三象限
        angle = (360 * Math.atan(non_dy / non_dx)) / (2 * Math.PI) - 180;
      }

      // 通过角度获取圆上点坐标  算出的坐标需要加上背景中心点坐标再减去按钮的半径
      this.dotX =
        (this.radius - this.d_radius) * Math.cos((angle * Math.PI) / 180) +
        this.zoreX -
        this.$refs.directionBtn.offsetHeight / 2;
      this.dotY =
        (this.radius - this.d_radius) * Math.sin((angle * Math.PI) / 180) +
        this.zoreY -
        this.$refs.directionBtn.offsetHeight / 2;
