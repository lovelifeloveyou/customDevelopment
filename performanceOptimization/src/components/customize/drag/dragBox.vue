  ...
  >
    ...
    <div v-else>
      ...
      <div
        ...
        v-if="[103,104].includes(Number(keymsg.rockerType))"
      >
        <img :src="keymsg.rockerType === 103 ? keyMroller_img[2] : keyMroller_img[3]" :width="keymsg.keyWidth" />
      </div>
    </div>
    <!-- dialog -->
...
export default {
  ...
  mounted() {
    ...
    console.log(this.editKeyboard,this.justSave)
    if (this.editKeyboard || this.justSave) {
      ...
      if ([101, 104, 102, 103].includes(this.keymsg.rockerType)) {
        this.keyInfo.keySize =
          Math.floor(
            (this.keyInfo.keyWidth - this.adaptModel.directionWidth) / 10
          ) + 5;
      } else {
        this.keyInfo.keySize =
          Math.floor((this.keyInfo.keyWidth - this.adaptModel.btnWidth) / 10) +
          5;
      }
    }
    console.log('执行')
      ...
      if (this.editKeyboard || this.justSave) {
        ...
        if (this.addNewCustomizeBtn) {
          ...
          this.dynamicBtnStyle = {
            left: this.adaptModel.btnLeft + "px",
            top: this.adaptModel.btnTop + "px",
            bottom: "",
          };
          this.directionStyle = {
          ...
          if ([101, 104, 102, 103].includes(Number(this.keymsg.rockerType))) {
            this.$refs.direction.style.left =
              this.adaptModel.directionLeft + "px";
            this.$refs.direction.style.top =
              this.adaptModel.directionTop + "px";
          } else {
            if (this.keymsg.keyStyle != 0) return;
        ...
        this.keyInfo.keyPressMode = 1;
        this.keyInfo.keySize = 5;
        if ([101, 104, 102, 103].includes(Number(this.keyInfo.rockerType))) {
          this.changeDirectionSizeStyle = {
            width: this.adaptModel.directionWidth + "px",
            height: this.adaptModel.directionWidth + "px",
          };
    ...
    closeDialog() {
      ...
      if (!this.editKeyboard) {
        ...
        if (filterRealValue) {
          ...
          this.keyInfo.keyHeight = this.keyInfo.keyWidth =
            filterRealValue.keyWidth;
          this.changeDirectionSizeStyle = {
